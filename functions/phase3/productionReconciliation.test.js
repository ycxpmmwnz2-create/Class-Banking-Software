...
test('reverify rejects a mismatched reviewed checkout before opening the credential or write artifacts',
  async () => {
    const { runReverifyMain, REVERIFY_EXIT_CODES } = await import('./reverify.js')
    const {
    ...
    const { exitCode, error } = await runReverifyMain([
    ...
    assert.equal(exitCode, REVERIFY_EXIT_CODES.AUTHORIZATION_REJECTED)
