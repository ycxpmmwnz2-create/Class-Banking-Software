# Firestore Rules Baseline Tests (Phase 0 + Phase 2A)
...
**current, unmodified, checked-in** `firestore.rules` via the Firebase
...
known-good baseline instead of relying on manual inspection. The repository
does not establish whether this exact rules file is currently deployed.
...
`isTeacher()` / `isStudent()` rules exactly as checked in.
...
- **Teacher** (`TEACHER_UID` hardcoded in the current rules): can read/write
  ...
  can read `studentAuthLogs`, cannot read/write the Phase 1
  `teachers/{uid}` document, and can read/write a generated Phase 1
  `classrooms/{classroomId}` root.
- **Unauthorized authenticated user**: denied on all of the above, and
  denied on `studentCredentials`, `teachers/{uid}`, and the generated Phase 1
  classroom root.
- **Student** (custom-token claims `role`/`classroomId`/`studentId`): can
  ...
  classroom data, `studentAuthLogs`, `studentCredentials`, `teachers/{uid}`,
  and the generated Phase 1 classroom root.
- **Unauthenticated user**: denied everywhere, including the Phase 1 teacher
  document and generated classroom root.

The Phase 2A additions intentionally describe the current checked-in rule
behavior rather than the future ownership model: `teachers/{uid}` has no
matching rule and is denied to every actor, while
`classrooms/{classroomId}` is covered by the existing broad `isTeacher()`
match and therefore remains readable and writable by the hardcoded teacher.
The ownership-based rules rewrite is deferred to Phase 3.
