...
const root = '/private/tmp/morgan-bank-conversation-candidate-20260906'
const runDir = `${root}/run-once`, budget = 2000000, baseline = 1016789
...
function fixtures() {
  ...
  return [
    ...
    { id: 'forty-students-ties', evidence: { ...structuredClone(base), transactions: students.map((_, i) => transaction(i, i < 2 ? 30 : 5)) } },
    { id: 'partial-history', evidence: { ...structuredClone(base), historyStart: '2026-08-31T12:00:00.000Z' } },
  ]
}
async function preflight() {
  const priorRaw = await readFile('/private/tmp/morgan-bank-isolated-integration-20260906/run-once/report.json')
